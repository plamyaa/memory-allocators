...
*malloc(size_t size)
{
    ...
    total_size = sizeof (header_t) + size; // Extending memory by incrementing
    block = sbrk(total_size);              // the program break (brk). It points
    if (block == (void *) -1) {            // to the end of the heap
        pthread_mutex_unlock(&global_malloc_lock);
        return NULL;
    }
